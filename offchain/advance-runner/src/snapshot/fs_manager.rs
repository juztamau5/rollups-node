    ...
    async fn validate(&self, snapshot: &Snapshot) -> Result<(), Self::Error> {
        if self.config.validation_enabled {
            ...
            let onchain_hash = crate::dapp_contract::get_template_hash(
                self.config.dapp_address.clone(),
                self.config.provider_http_endpoint.clone().unwrap(),
            )
            .await
            .context(OnchainSnafu)?;

            match offchain_hash == onchain_hash {
                true => Ok(()),
                false => Err(FSSnapshotError::HashMismatchError),
            }
        } else {
            tracing::info!("snapshot validation disabled, accepting snapshot");
            Ok(())
        }
    }
}
